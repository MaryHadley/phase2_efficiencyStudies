...
void debug_loop_ZplusY_effStudies (string file){//, string file2){
...
///////////////////////////
  ...
  int big4MuVtxProbVecSizeGreaterThan1Count = 0;
  ...
  for(int iEntry=0; iEntry<entries; iEntry++) {
   ...
   // std:cout << "TREE->denominator_ZplusY" << TREE->denominator_ZplusY << std::endl;
    ...
    std::cout << "run_number:  " <<  TREE->run_number->at(0) << std::endl;
    std::cout << "event_number: " << TREE->event_number->at(0) << std::endl;
    std::cout << "lumi_section: " << TREE->lumi_section->at(0) << std::endl;
    std::cout << "////////////////////////////////////////////////////" << std::endl;
...
//    std::cout <<"temp_Z_mass.size()" <<  temp_Z_mass.size() << std::endl; 
   ...
   auto lepton4_pt_vec = TREE->lepton4_pt;
   std::vector<int> big4MuVtxProbPassIndexVec;
   ...
   std::cout << "max4MuVtxProbIndex:  " << max4MuVtxProbIndex << std::endl;  //Pick out the index of the quad with the greatest big4MuVtxProb --> this is the quad we will consider for these studies.
    double  max4MuVtxProb = *std::max_element(big4MuVtxProbVec->begin(),big4MuVtxProbVec->end());
    std::cout << "max4MuVtxProb:   " << max4MuVtxProb << std::endl;
    std::cout << "big4MuVtxProbVec->size():  " << big4MuVtxProbVec->size() << std::endl;
    std::cout << "lepton4_pt_vec->size():  " << lepton4_pt_vec->size() << std::endl;
    if (lepton4_pt_vec->size() != big4MuVtxProbVec->size()){
      std::cout << "DONKEY" << std::endl; //Sanity check passed, their sizes are always the same, we never get DONKEY printed to the screen
    }  
    std::cout << "TREE->numZplusYCandInEvent->at(0):  " << TREE->numZplusYCandInEvent->at(0) << std::endl;
    if (big4MuVtxProbVec->size() > 1){
      big4MuVtxProbVecSizeGreaterThan1Count++;
      std::cout << "POODLES" << std::endl;
    } 
    for (int i =0; i < big4MuVtxProbVec->size(); i++){
      std::cout << "big4MuVtxProbVec->at(i):  " << big4MuVtxProbVec->at(i) << std::endl;
      if (big4MuVtxProbVec->at(i) > big4MuVtx_Prob_Cut){
         big4MuVtxProbPassIndexVec.push_back(i);
      
      }
     }
     
    int indexToUse = -99;
    double lepton4_pt_comparator = -100000;
    
    for (int ii=0; ii < big4MuVtxProbPassIndexVec.size(); ii++){
      std::cout << "big4MuVtxProbPassIndexVec.at(ii):  " << big4MuVtxProbPassIndexVec.at(ii) << std::endl;
      std::cout << "lepton4_pt_vec->at(big4MuVtxProbPassIndexVec.at(ii)):  " << lepton4_pt_vec->at(big4MuVtxProbPassIndexVec.at(ii)) << std::endl;
      if (lepton4_pt_vec->at(big4MuVtxProbPassIndexVec.at(ii)) > lepton4_pt_comparator){
        indexToUse = big4MuVtxProbPassIndexVec.at(ii);
        lepton4_pt_comparator = lepton4_pt_vec->at(big4MuVtxProbPassIndexVec.at(ii));
        
      }
    
    }
    
    std::cout << "indexToUse:  " << indexToUse << std::endl;
    std::cout << "////////////////////////////////////////////////////////////////////" << std::endl;
...
//         // std::cout << (lepton1 + lepton2).M()<< std::endl; 
         ...
         //Fill the fifteenth bin of the cutflow histo here. This is after the upsi rapidity cut.
           ...
           std::cout << "event_number: " << TREE->event_number->at(0) << std::endl;
           std::cout << "lumi_section:  " << TREE->lumi_section->at(0) << std::endl;
           std:: cout << "corresponding to quad with max4MuVtxProb:  " << max4MuVtxProb << std::endl;
...
std::cout << "big4MuVtxProbVecSizeGreaterThan1Count:  " << big4MuVtxProbVecSizeGreaterThan1Count << std::endl;
///////////////////////
//////    M C    //////
///////////////////////
